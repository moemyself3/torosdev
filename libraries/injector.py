...
class Injector:
    ...
    cutout_size : `int`, default=30
        size of candidate star square cutout sides NOTE: final shape is trimmed 
            cutout_size - 2px by cutout_size - 2px
    ...
    def __init__(self, image, threshold=100, 
        ...
        try:
            self.image_median = self.image.header['SKY']
        except:
            self.image_median = median

        self.threshold = std * 7.0
        ...
        if os.path.isfile(Configuration.CALIBRATION_DIRECTORY + "mask.fits"):
            self.mask = fits.getdata(Configuration.CALIBRATION_DIRECTORY + "mask.fits").astype('bool')
    ...
    def make_candidate_starlist(self):
        ...
        cleanarr, crmask = cosmicray_lacosmic(
                                        self.image.data,
                                        sigclip=3,
                                        gain=Configuration.GAIN,
                                        readnoise=Configuration.READOUT_NOISE,
                                        satlevel=Configuration.PEAKMAX
                                        )

        mask = self.mask | crmask

        median_subtracted_img = self.image.data - self.image_median

        Utils.log("Start peaks weaks DAOStarFinder", "debug")
        peaks_table = peaks_DAOStarFinder(median_subtracted_img, mask=mask)
        weaks_table = weaks_DAOStarFinder(median_subtracted_img, mask=mask)
        ...
        # keep dimmer stars using Magnitude cutoff
        #peaks_table = peaks_table[(peaks_table['mag']+Configuration.MAG_OFFSET > Configuration.UPPER_MAG_LIMIT)]
        #weaks_table = weaks_table[(weaks_table['mag']+Configuration.MAG_OFFSET > Configuration.UPPER_MAG_LIMIT)]
        ...
        flux = lonely_peaks_table['flux']
        npix = lonely_peaks_table['npix']
        Fsky = self.image_median
        Fdark = 0 # Dark has already been removed
        Fread = Configuration.READOUT_NOISE
        N = np.sqrt(G*flux + npix*(G*Fsky+ Fdark + Fread**2))/G
        SNR = flux/N
        lonely_peaks_table['SNR'] = SNR

        # add RA DEC coordinates to table
        wcs = WCS(self.image.header)
        coords = wcs.all_pix2world(lonely_peaks_table['x_peak'], lonely_peaks_table['y_peak'], 0)
        sky_coords = SkyCoord(coords[0], coords[1], unit=(u.deg, u.deg))
        lonely_peaks_table['ra'] = sky_coords.ra.deg
        lonely_peaks_table['dec'] = sky_coords.dec.deg
    ...
    def make_epsf(self, psf_stats=True, verb=True):
        ...
        # stronger sources
        lonely_peaks_table = lonely_peaks_table[(lonely_peaks_table['SNR']> 10.0)]
        ...
        # using quadratic smoothing kernel renders cleaner models
        ...
        # Some corrections needed for better models
        # trim model to remove 2 pixels around border
        self.epsf_resized = self.epsf_resized[3:-3, 3:-3]
        # fill values less than 0 with 0
        self.epsf_resized[self.epsf_resized < 0] = 0
        # Normalize psf to 1.0
        self.epsf_resized = self.epsf_resized / np.sum(self.epsf_resized)
        ...
        if psf_stats == True:
            ...
            self.psf_magnitude = Configuration.MAG_OFFSET - 2.5*np.log10(
                    self.psf_stats.sum )
    ...
    def inject(self, position, magnitude, image):#=None):
        ...
        target_counts = 10**((Configuration.MAG_OFFSET-magnitude)/2.5) 
