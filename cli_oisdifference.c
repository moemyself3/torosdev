...
{
    char  arg_scifile[1024], arg_dfilename[1024], arg_ref[1024], arg_refstars[1024];
    // Parse arguments ./a.out scifile dfilename
    // check for more than 1 argument
    if (argc > 1 && argc == 5){ // zero argument is the program name
        strcpy(arg_scifile, argv[1]); // first arg is scifile
        strcpy(arg_dfilename, argv[2]); // second arg is dfilename
        strcpy(arg_ref, argv[3]);
        strcpy(arg_refstars, argv[4]);
    } else {
        if (argc > 1 ){
            printf("Expected 4 args. %d args given. Ignoring given values. Using defaults values.\n", argc-1);
        }
        strcpy(arg_scifile, "img.fits");
        strcpy(arg_dfilename, "dimg.fits");
        strcpy(arg_ref, "ref.txt");
        strcpy(arg_refstars, "refstars.txt");
    }
    ...
    strcpy(ref, "./"); // writes string ./ //
    strcat(ref, arg_ref); // make string for ./ref.txt //
    ...
    strcpy(refstars, "./"); // writes string ./ //
    strcat(refstars, arg_refstars); // make string for ./refstars.txt //
...
// Now we can do the final convolution // 
    ...
    dfilename = strdup(arg_dfilename);
    
    //sprintf(dfilename, "./d%s",sciname);
    remove(dfilename);
    status = 0;
    printf("dfilename = %s", dfilename); 
